        ...
        if(usuario.fundador || usuario.administrador) {
            if(razaou.length < 1) {
                ...
                sql.Servidores.findOne({_id: message.guild.id}, function (erro, servidor) {
                    ...
                    message.channel.send(`${s} \`${message.author.tag}\` utilize o comando: \`${servidor.prefix}setvip/setvp <usuário>\`.`).then(msg => msg.delete(5000));
                })
            } else {
                ...
                sql.Usuarios.findOne({_id: member.id}, function (erro, usuario) {
                    ...
                    if (usuario) {
                        if (usuario.vip) {
                            ...
                            message.channel.send(`${r} \`${message.author.tag}\` você removeu o cargo \`Vip\` do usuário \`${member.tag}\`.`).then(msg => msg.delete(5000));

                        } else {
                            usuario.vip = true
                            usuario.save()

                            message.channel.send(`${a} \`${message.author.tag}\` você adicionou o cargo \`Vip\` no usuário \`${member.tag}\`.`).then(msg => msg.delete(5000));
                        }
                    } else {
