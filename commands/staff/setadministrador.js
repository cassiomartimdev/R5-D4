...
sql.Usuarios.findOne({_id: message.author.id}, function (erro, usuario) {
  ...
  if(usuario.fundador) {
    ...
    message.channel.send(`${s} \`${message.author.tag}\` utilize o comando: \`${servidor.prefix}setadministrador/seta <usuário>\`.`).then(msg => msg.delete(5000));
        })
        } else {
...
sql.Usuarios.findOne({_id: member.id}, function (erro, usuario) {
  ...
  message.channel.send(`${r} \`${message.author.tag}\` você removeu o cargo \`Administrador\` do usuário \`${member.tag}\`.`).then(msg => msg.delete(5000));

              } else {
                usuario.administrador = true
                usuario.save()
  
                message.channel.send(`${a} \`${message.author.tag}\` você adicionou o cargo \`Administrador\` no usuário \`${member.tag}\`.`).then(msg => msg.delete(5000));
              }
            } else {
